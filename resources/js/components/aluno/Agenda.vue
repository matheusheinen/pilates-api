  ...
  <div class="space-y-6 pb-24">
    ...
    <div class="bg-[#1e1e1e] rounded-2xl p-4 border border-gray-800 shadow-lg">
        <div class="flex justify-between items-center mb-4 px-1">
            ...
            <h3 class="text-white font-bold text-lg capitalize tracking-wide">{{ mesAnoFormatado }}</h3>
            ...
            <div class="p-4 flex items-center justify-between">
                ...
                <div class="flex flex-col items-end gap-2">
                    ...
                    <button
                        ...
                        @click.stop="abrirReagendamento(aula)"
                        class="text-[10px] font-semibold text-teal-500 bg-teal-500/10 hover:bg-teal-500/20 px-2 py-1 rounded transition border border-teal-500/20 z-10 relative"
                    >
                        Reagendar
                    </button>
                </div>
    ...
    <Teleport to="body">
        ...
        <div v-if="modalReagendar" class="fixed inset-0 z-[9999] flex flex-col justify-end">

            <div class="absolute inset-0 bg-black/80 backdrop-blur-sm transition-opacity" @click="fecharReagendamento"></div>

            <div class="relative bg-[#181818] w-full rounded-t-2xl border-t border-gray-700 shadow-2xl flex flex-col max-h-[85vh] overflow-hidden safe-area-pb">
...
const diasSemana = computed(() => {
    ...
    for (let i = 0; i < 7; i++) {
        ...
        dias.push({
            ...
            letra: format(data, 'EEEEE', { locale: ptBR }).toUpperCase(),
        });
    }
    return dias;
});
...
const podeReagendar = (aula) => {
    ...
    const diffHoras = (dataAula - agora) / 36e5; // Diferença em horas

    // Permite reagendar se faltar mais de 24h e status válido
...
const confirmarReagendamento = async (vaga) => {
    // Isso aqui requer que o AlertMobile esteja importado no Aluno.vue (Pai)
